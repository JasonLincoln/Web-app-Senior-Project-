...
function showAllUsers(data){
    ...
    data.forEach(item => {
        ...
        li.innerHTML = `ID: ${item.id}, Username: ${item.username}, Email: ${item.email}, <br>
        Hashed Password: ${item.hashed_password}, Display Name: ${item.display_name}, Pronouns: ${item.pronouns}, <br>
        Gender: ${item.gender}, Biography: ${item.biography}, Profile Picture: ${item.profile_url}, <br>
        Role: ${item.role}, Active: ${item.is_active} <br> <br>`;
...
const allSkillsButton = document.getElementById('allUsersButton');
...
const allAchivements = document.getElementById('allAchivements');
const getAllAchivementsEndpoint = '/admin/Achievement';
const allAchivementsButton = document.getElementById('allAchivementsButton');

allAchivementsButton.addEventListener('click', () => {
    fetch(getAllAchivementsEndpoint)
    .then(response => response.json)
    .then(data => {
        showAllAchivements(data);
    })
})

function showAllAchivements(data){
    allAchivements.innerHTML = "";
    data.forEach(item => {
        const li = document.createElement('li');
        li.innerHTML = `ID: ${item.id}, Name: ${item.name}, Description: ${item.description}, Image URL: ${item.img_url}`;
        allAchivements.appendChild(li);
    });
}
